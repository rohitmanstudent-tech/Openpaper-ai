...
import { Bot, Star, Download, Loader2, ArrowLeft, Search, Check, Trash2, RefreshCw } from "lucide-react"
...
export default function MarketplaceAgentsPage() {
  ...
  const getStatus = (itemId: string) => {
    const inst = installed.find((i) => i.item_id === itemId)
    return inst?.status || "not_installed"
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white flex items-center gap-3">
            <Bot className="h-8 w-8 text-accent-400" /> Agent Marketplace
          </h1>
          <p className="text-muted-foreground mt-1">AI agents for specialized tasks and automation</p>
        </div>
      </div>
              ...
              <Card key={item.id} className="border-white/10 flex flex-col">
                ...
                <div className="px-6 pb-4 pt-0">
                  {status === "installed" ? (
                    ...
                    <Button size="sm" className="w-full" onClick={() => updateItem(item.id)}>
