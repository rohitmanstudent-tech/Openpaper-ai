...
import { Wrench, Star, Download, Loader2, ArrowLeft, Search, Trash2, RefreshCw } from "lucide-react"
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white flex items-center gap-3">
            <Wrench className="h-8 w-8 text-amber-400" /> Tool Marketplace
          </h1>
          <p className="text-muted-foreground mt-1">Utility tools and integrations for your workflows</p>
        </div>
      </div>

      <div className="relative max-w-md">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
        <Input placeholder="Search tools..." className="pl-10" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
