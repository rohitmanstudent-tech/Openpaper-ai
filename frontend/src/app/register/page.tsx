  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const data = await api.post<{ access_token: string; user: any }>(
        "/auth/register", { email, username, password, full_name: name }
      );
    ...
    <div className="flex min-h-screen items-center justify-center bg-base-950 px-4">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-xl font-semibold text-base-100">Create account</h1>
          <p className="mt-1 text-sm text-base-500">Get started with OpenPaper AI</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Input id="username" label="Username" value={username}
            onChange={(e) => setUsername(e.target.value)} placeholder="johndoe" required />
        ...
        <p className="mt-6 text-center text-sm text-base-500">
          ...
          <Link href="/login" className="text-accent-400 hover:text-accent-300 transition-colors">Sign in</Link>
        </p>
      </div>
    </div>
  );
